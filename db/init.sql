...
CREATE TABLE IF NOT EXISTS company.items (
...
CREATE TABLE IF NOT EXISTS company.images (
...
ALTER TABLE company.images
ADD CONSTRAINT fk_item
FOREIGN KEY (parent_id)
REFERENCES company.items(pk)
ON UPDATE CASCADE
ON DELETE NO ACTION;


ALTER TABLE company.items
  ADD CONSTRAINT fk_category
  FOREIGN KEY (category_id)
  REFERENCES company.categories(pk) MATCH SIMPLE
  ON UPDATE CASCADE
  ON DELETE NO ACTION;CREATE VIEW company.get_item AS
SELECT
  item.pk,
  item.title,
  item.description,
...
FROM company.items AS item
JOIN company.images AS image
ON item.pk = image.parent_id
GROUP BY
  item.pk, item.title, item.description, item.price, item.discount, item.category_id;WITH created_category AS (
  INSERT INTO company.categories(title, image)
  VALUES (
    'regular shoes',
...
created_item AS (
  INSERT INTO company.items(title, description, price, discount, category_id, sizes) 
  VALUES (
    'Shoe',
    'It is just a shoe',
    45,
...
INSERT INTO company.images(is_main_image, src, parent_id)
SELECT 
  TRUE,
...
FROM created_item;
  ...
  WITH existing_category AS (
    SELECT * FROM company.categories WHERE title = 'regular shoes'
  ),
  created_item AS (
    INSERT INTO company.items(title,description, price, discount, category_id, sizes) VALUES(
      'Graceful Wedges',
      'Graceful Wedges',
      168,
      136,
